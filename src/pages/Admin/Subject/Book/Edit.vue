...
<script>
  export default {
    ...
    methods:{
      ...
      saveBooks(){
        this.$store.dispatch('subject/storeBooks', {
          id: this.$route.params.id,
          books: this.books,
        }).then(response => {
          this.$router.push('/admin/subjects/' +this.$route.params.id+ '/books');
        }).catch(error => {
          this.errorNotify();
        });
    ...
    computed: {
      ...
      BooksArray() {
        return this.$store.getters['subject/BooksArray'];
      },
      books_val: {
        get () {
    ...
    beforeMount() {
      ...
      this.$store.dispatch("subject/getBooksArray", {
        id: this.$route.params.id,
      });
    }
  }
